...
n = 10; % TODO: WAS IST M UND N EIGENTLICH?? 
...
% Arnoldi iteration
[V,H] = Arnoldi(Z,w,m+1);
[V2,H2] = Arnoldi(Z+ 0.01 * rand(size(Z)),w,m+1);
% Updating procedure
Hup = updating(Z,w,'PR');
Hup2 = updating(Z+ 0.01 * rand(size(Z)),w,'PR');
%% plot computed zeros

% Annahme: A, B, C und D sind Ihre Matrizen (Sie müssen sie entsprechend definieren)
A = Hup2; % Beispiel für A
...
for i = 1:10
    ...
    scatter(real(eigenvalues_B_i), y_values_B_i, 'ko');
    
    % Plot der Eigenwerte von C_i in Grün
    scatter(real(eigenvalues_C_i), y_values_C_i, 'bx');
    
    % Plot der Eigenwerte von D_i in Pink
    scatter(real(eigenvalues_D_i), y_values_D_i, 'ko');
...
title({'Aufteilung in rot, Arnoldi in Blau,','Ergebnisse mit Störung (0.01*rand) mit x'});
xlabel('Reale Achse');
ylabel('i');
