...
close all
clearvars
n = 100; s = 1; nd = [n n]; a0 = 0; same = 1;
ab = r_jacobi(n);
iter = 100;
%gamma_values = 10.^(-6:10);
gamma_values = [1];

max_cond_Arn_iter = zeros(1, iter); % Initialisiere Array für maximale EW-Konditionszahlen von Arnoldi
max_cond_Auf_iter = zeros(1, iter); % Initialisiere Array für maximale EW-Konditionszahlen von Aufteilung
max_cond_Sti_iter = zeros(1, iter); % Initialisiere Array für maximale EW-Konditionszahlen von Stieltjes
...
for gamma_index = 1:length(gamma_values)
    ...
    for k = 1:iter
        ...
        [X_Stieltjes, D_Stieltjes] = sobzeros_mine(k,k,Stieltjes);
        ...
        D_Aufteilung = sort(D_Aufteilung, 'ComparisonMethod','real'); % bei HH entstehen imaginäre Anteile
